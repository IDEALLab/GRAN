...
class GNN(nn.Module):
    def __init__(self,
        ...
        self.update_func = nn.ModuleList(
            [nn.GRUCell(input_size=self.msg_dim, hidden_size=self.node_state_dim) for _ in range(self.num_layer)])

        self.msg_func = nn.ModuleList([
            nn.Sequential(*[
                nn.Linear(self.node_state_dim + self.edge_feat_dim, self.msg_dim),
                nn.ReLU(),
        ...
        if self.has_attention:
            self.att_head = nn.ModuleList([
                nn.Sequential(*[
                    nn.Linear(self.node_state_dim + self.edge_feat_dim, self.att_hidden_dim),
                    nn.ReLU(),
        ...
        if self.has_graph_output:
            ...
            self.graph_output_head = nn.Sequential(*[nn.Linear(self.node_state_dim, self.graph_output_dim)])

    def _prop(self, state, edge, edge_feat, layer_idx=0):
        ### compute message
        state_diff = state[edge[:, 0], :] - state[edge[:, 1], :]  # (M, D)
        if self.edge_feat_dim > 0:
            edge_input = torch.cat([state_diff, edge_feat], dim=1)  # (M, D+D')
        else:
            edge_input = state_diff
        # print('edge_input.shape=', edge_input.shape)

        msg = self.msg_func[layer_idx](edge_input)  # (M, msg_dim)
        # print('msg.shape=', msg.shape)

        ### attention on messages
        if self.has_attention:
            att_weight = self.att_head[layer_idx](edge_input)  # (M, msg_dim)
            msg = msg * att_weight  # (M, msg_dim)

        ### aggregate message by sum
        state_msg = torch.zeros(state.shape[0], msg.shape[1]).to(state.device)  # (N, msg_dim)
        scatter_idx = edge[:, [1]].expand(-1, msg.shape[1])  # (M, msg_dim)
        state_msg = state_msg.scatter_add(0, scatter_idx, msg)  # how does this work???
    ...
    """
        ...
        if self.has_graph_output:
            ...
            const = const.scatter_add(0, graph_idx, torch.ones(node_output.shape[0]).to(node_feat.device))
...
class GRANMixtureBernoulli(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.max_node_label = config.dataset.max_node_label  #XD: Note: it starts from 0! So shape = max_node_label + 1.
        ...
        self.edge_weight = config.model.edge_weight if hasattr(config.model, 'edge_weight') else 1.0
        ...
        self.output_theta = nn.Sequential(
            nn.Linear(self.hidden_dim + self.max_node_label + 1, self.hidden_dim),
        ...
        self.node_label_pred_func = nn.Sequential(
            nn.Linear(self.hidden_dim + self.max_node_label + 1, self.hidden_dim),
            nn.ReLU(inplace=True),
            nn.Linear(self.hidden_dim, self.hidden_dim),
            nn.ReLU(inplace=True),
            nn.Linear(self.hidden_dim, self.max_node_label + 1))

        if self.dimension_reduce:
            self.embedding_dim = config.model.embedding_dim
            self.decoder_input = nn.Sequential(nn.Linear(self.max_num_nodes, self.embedding_dim))
        else:
        ...
        self.decoder = GNN(
            ...
            node_state_dim=self.hidden_dim + self.max_node_label + 1,
        ...
        self.node_label_loss = nn.BCEWithLogitsLoss()  # reduction='mean'

    def _inference(self, A_pad=None, node_label=None, edges=None, node_idx_gnn=None, node_idx_feat=None, att_idx=None):
        """ generate adj in row-wise auto-regressive fashion """

        # print('=' * 10, 'inference', '=' * 10)

        B, C, N_max, _ = A_pad.shape  # B=16, C=1, N_max=500
        H = self.hidden_dim  # H=512
        K = self.block_size  # K=1
        A_pad = A_pad.view(B * C * N_max, -1)  # (8000, 500)
        # print('B = {}, C = {}, N_max = {}, H = {}, K = {}, A_pad.shape = {}'.format(B, C, N_max, H, K, A_pad.shape))
        node_label = node_label.view(B * C * N_max, -1)  # should be (8000, 9)

        if self.dimension_reduce:
            node_feat = self.decoder_input(A_pad)  # BCN_max X H, (8000, 512)
        else:
        ...
        ### GNN inference
        '========XD========'
        # concatenate node_feat with node_label
        node_feat = torch.cat((node_feat, node_label), 1)
        # print('after concatenate: node_feat.shape = {}'.format(node_feat.shape))  # should be (8000, 521)
        '========XD========'

        # pad zero as node feature for newly generated nodes (1st row)
        node_feat = F.pad(node_feat, (0, 0, 1, 0), 'constant', value=0.0)  # (BCN_max + 1) X N_max, (8001, 512)
        # print('pad one row above: node_feat.shape = {}'.format(node_feat.shape))
        ...
        if self.has_rand_feat:
            ...
            idx_new_node = (att_idx[[edges[:, 0]]] > 0).long() + (att_idx[[edges[:, 1]]] > 0).long()
            idx_new_node = idx_new_node.byte().squeeze()
            att_edge_feat[idx_new_node, :] = torch.randn(idx_new_node.long().sum(), att_edge_feat.shape[1]).to(
                node_feat.device)
        else:
        ...
        # print('node_idx_feat.shape = {}'.format(node_idx_feat.shape))  # (396)
        # print('decoder input shapes: {}, {}, {}'.format(node_feat[node_idx_feat].shape, edges.shape,
        #                                                 att_edge_feat.shape))  # (396, 512), (1520, 2), (1520, 128)
        node_state = self.decoder(node_feat[node_idx_feat], edges, edge_feat=att_edge_feat)
        print('node_state.shape = {}'.format(node_state.shape))  # （396，512）
        ...
        # print('log_theta.shape = {}'.format(log_theta.shape))  #（396, 20)
        log_alpha = self.output_alpha(diff)  # B X (tt+K)K
        # print('log_alpha.shape = {}'.format(log_alpha.shape))  #（396, 20)
        ...
        node_label_pred = self.node_label_pred_func(node_state)
        print('node_label_pred.shape = {}'.format(node_label_pred.shape))
        return log_theta, log_alpha, node_label_pred
    ...
    def _sampling(self, B):
        ...
        with torch.no_grad():
            ...
            mod_val = (N - K) % S
            # print('K = {}, S = {}, H = {}, N = {}, mod_val = {}'.format(K, S, H, N, mod_val))

            if mod_val > 0:
                N_pad = N - K - mod_val + int(np.ceil((K + mod_val) / S)) * S
            else:
                N_pad = N
            # print('N_pad = {}'.format(N_pad))  # 500

            A = torch.zeros(B, N_pad, N_pad).to(self.device)
            dim_input = self.embedding_dim if self.dimension_reduce else self.max_num_nodes
            # print('dim_input = {}'.format(dim_input))  # 512

            ### cache node state for speed up
            node_state = torch.zeros(B, N_pad, dim_input).to(self.device)  # (13, 500, 512)
            node_label = torch.zeros(B, N_pad, self.max_node_label + 1).to(self.device)

            for ii in range(0, N_pad, S):
                # for ii in range(0, 500, S):
                jj = ii + K
                ...
                node_label[:, ii:, :] = .0  #XD: this line is actually redundant

                if ii >= K:
                    if self.dimension_reduce:
                        node_state[:, ii - K:ii, :] = self.decoder_input(A[:, ii - K:ii, :N])
                    else:
                        node_state[:, ii - K:ii, :] = A[:, ii - S:ii, :N]  # XD: ii-K vs. ii-S?
                        # XD: N_pad vs. N?
                else:
                    if self.dimension_reduce:
                ...
                # print('node_state.shape = {}'.format(node_state.shape))  # (13, 500, 512)

                node_state_in = F.pad(
                    torch.cat((node_state[:, :ii, :], node_label[:, :ii, :]), dim=2), (0, 0, 0, K),
                    'constant',
                    value=.0)  # XD: padding makes spaces for the K rows to be generated. See node_state_out.
                # print('node_state_in.shape = {}'.format(node_state_in.shape))  # (B, jj, 512)

                ### GNN propagation
                adj = F.pad(A[:, :ii, :ii], (0, K, 0, K), 'constant', value=1.0)  # B X jj X jj
                adj = torch.tril(adj, diagonal=-1)
                adj = adj + adj.transpose(1, 2)
                edges = [adj[bb].to_sparse().coalesce().indices() + bb * adj.shape[1] for bb in range(B)]
                edges = torch.cat(edges, dim=1).t()

                att_idx = torch.cat([torch.zeros(ii).long(), torch.arange(1, K + 1)]).to(self.device)
                att_idx = att_idx.view(1, -1).expand(B, -1).contiguous().view(-1, 1)

                if self.has_rand_feat:
                    # create random feature
                    att_edge_feat = torch.zeros(edges.shape[0], 2 * self.att_edge_dim).to(self.device)
                    idx_new_node = (att_idx[[edges[:, 0]]] > 0).long() + (att_idx[[edges[:, 1]]] > 0).long()
                    idx_new_node = idx_new_node.byte().squeeze()
                    att_edge_feat[idx_new_node, :] = torch.randn(idx_new_node.long().sum(), att_edge_feat.shape[1]).to(
                        self.device)
                ...
                # print('decoder input shapes: {}, {}, {}'.format(
                #     node_state_in.view(-1, H).shape, edges.shape,
                #     att_edge_feat.shape))  # (B*jj, 512), (B*jj, 2), (B*jj, 128)
                node_state_out = self.decoder(
                    node_state_in.view(-1, H + self.max_node_label + 1), edges, edge_feat=att_edge_feat)
                node_state_out = node_state_out.view(B, jj, -1)
                # print('node_state_out.shape = {}'.format(node_state_out.shape))  # (B, jj, 512)
                # print('check if node_state_out gets updated at each time step', node_state_out[:, 0, :10])

                '========XD========'
                node_label_pred = self.node_label_pred_func(node_state_out.view(-1, H + self.max_node_label + 1))
                # print('node_label_pred.shape = {}'.format(node_label_pred.shape))
                node_label_pred = node_label_pred.view(B, jj, self.max_node_label + 1)
                for bb in range(B):
                    ans = torch.argmax(node_label_pred[bb, :, :], dim=1)
                    for jjj in range(jj):
                        node_label[bb, jjj, ans[jjj]] = 1.0
                '========XD========'
                ...
                # print('diff.shape = {}'.format(diff.shape))  # (20, jj*K, 512)
                diff = diff.view(-1, dim_input + self.max_node_label + 1)
                # print('diff.shape = {}'.format(diff.shape))  # (B*jj*K, 512)
                log_theta = self.output_theta(diff)  # (B*jj*K, L)
                # print('log_theta.shape = {}'.format(log_theta.shape))
                log_alpha = self.output_alpha(diff)  # (B*jj*K, L)
                ...
                log_alpha = log_alpha.view(B, -1, self.num_mix_component)  # B X K X (ii+K)? (B, jj*K, L)
                prob_alpha = F.softmax(log_alpha.mean(dim=1), -1)
                # print('prob_alpha.shape = {}'.format(prob_alpha.shape))  # (B, L)
                alpha = torch.multinomial(prob_alpha, 1).squeeze(dim=1).long()
                # print('alpha.shape = {}'.format(alpha.shape))  # B
    ...
    """
        ...
        node_label_gt = input_dict['node_label_gt'] if 'node_label_gt' in input_dict else None
        ...
        subgraph_idx_base = input_dict["subgraph_idx_base"] if "subgraph_idx_base" in input_dict else None
        ...
        if not is_sampling:
            ...
            ### compute adj loss
            log_theta, log_alpha, node_label_pred = self._inference(
            ...
            adj_loss = mixture_bernoulli_loss(label, log_theta, log_alpha, self.adj_loss_func, subgraph_idx,
                                              subgraph_idx_base, self.num_canonical_order)

            # return adj_loss

            node_label_loss = self.node_label_loss(node_label_pred, node_label_gt)
            print('node_label_gt.shape = {}'.format(node_label_gt.shape))
            print('adj_loss/node_label_loss = {:.2f}'.format(adj_loss / node_label_loss))
            joint_loss = adj_loss + 0.5 * node_label_loss
            return joint_loss
        else:
            A, node_label = self._sampling(batch_size)
            ...
            node_label_list = [node_label[ii, :num_nodes[ii], :] for ii in range(batch_size)]
            # from one-hot to integers
            node_label_list = [torch.argmax(node_label, dim=1) for node_label in node_label_list]
            # print(A_list[0], A_list[0].shape)
            # print(node_label_list[0], node_label_list[0].shape)
            # return
            return A_list, node_label_list


def mixture_bernoulli_loss(label,
                           log_theta,
                           log_alpha,
                           adj_loss_func,
                           subgraph_idx,
