...
PRINT_TIME = False
__all__ = [
    'clean_graphs', 'degree_stats', 'clustering_stats', 'orbit_stats_all', 'spectral_stats', 'eval_acc_lobster_graph'
]
...
def orca(graph):
    ...
    output = sp.check_output(['./utils/orca/orca', 'node', '4', 'utils/orca/tmp.txt', 'std'])
    output = output.decode('utf8').strip()
    idx = output.find(COUNT_START_STR) + len(COUNT_START_STR) + 2
    output = output[idx:]
    node_orbit_counts = np.array(
        [list(map(int,
                  node_cnts.strip().split(' '))) for node_cnts in output.strip('\n').split('\n')])
...
def mean_degree_stats(graph_ref_list, graph_pred_list):
    # in case an empty graph is generated
    graph_pred_list_remove_empty = [G for G in graph_pred_list if not G.number_of_nodes() == 0]

    sample_ref = [np.mean(list(dict(nx.degree(G)).values())) for G in graph_ref_list]
    sample_pred = [np.mean(list(dict(nx.degree(G)).values())) for G in graph_pred_list_remove_empty]

    r = (min(min(sample_ref), min(sample_pred)), max(max(sample_ref), max(sample_pred)))
    ...
    mmd_dist = compute_mmd([hist_ref], [hist_pred], kernel=gaussian)  # remember to put those []!
    return mmd_dist
...
def max_degree_stats(graph_ref_list, graph_pred_list):
    ...
    sample_ref = [np.max(list(dict(nx.degree(G)).values())) for G in graph_ref_list]
    sample_pred = [np.max(list(dict(nx.degree(G)).values())) for G in graph_pred_list_remove_empty]
...
def mean_centrality_stats(graph_ref_list, graph_pred_list):
    # in case an empty graph is generated
    graph_pred_list_remove_empty = [G for G in graph_pred_list if not G.number_of_nodes() == 0]

    sample_ref = [np.mean(list(nx.degree_centrality(G).values())) for G in graph_ref_list]
    sample_pred = [np.mean(list(nx.degree_centrality(G).values())) for G in graph_pred_list_remove_empty]
...
def assortativity_stats(graph_ref_list, graph_pred_list):
    ...
    sample_ref = [nx.degree_assortativity_coefficient(G) for G in graph_ref_list]
    sample_pred = [nx.degree_assortativity_coefficient(G) for G in graph_pred_list_remove_empty]
...
def mean_degree_connectivity_stats(graph_ref_list, graph_pred_list):
    ...
    sample_ref = [np.mean(list(nx.average_degree_connectivity(G).values())) for G in graph_ref_list]
    sample_pred = [np.mean(list(nx.average_degree_connectivity(G).values())) for G in graph_pred_list_remove_empty]
...
def draw_hists(test_graphs, baseline_graphs, generated_graphs):
    # Do not remove isolates from baseline_graphs due to the use of nx.subgraph. 
    # Otherwise, an Error rises: Frozen graph can't be modified.
    
    # number of nodes
    stats_test = [len(G) for G in test_graphs]
    stats_baseline = [len(G) for G in baseline_graphs]
    stats_generated = [len(G) for G in generated_graphs]
    print(len(stats_test), len(stats_baseline), len(stats_generated))
    print('number of nodes: baseline', stats_baseline)
    print('number of nodes: generated', stats_generated)

    plt.figure()
    sns.kdeplot(stats_test, legend=False, color='g')
    sns.kdeplot(stats_baseline, legend=False, color='orange')
    sns.kdeplot(stats_generated, legend=False, color='magenta')

    plt.legend(['Ground Truth', 'Baseline', 'Generated'])
    plt.title('number_of_nodes')
    plt.xlabel('value')
    plt.savefig('number_of_nodes')

    # mean degree
    stats_test = [np.mean(list(dict(nx.degree(G)).values())) for G in test_graphs]
    stats_baseline = [np.mean(list(dict(nx.degree(G)).values())) for G in baseline_graphs]
    stats_generated = [np.mean(list(dict(nx.degree(G)).values())) for G in generated_graphs]
    ...
    plt.xlabel('value')
    plt.savefig('mean_degree')

    # max degree
    stats_test = [np.max(list(dict(nx.degree(G)).values())) for G in test_graphs]
    stats_baseline = [np.max(list(dict(nx.degree(G)).values())) for G in baseline_graphs]
    stats_generated = [np.max(list(dict(nx.degree(G)).values())) for G in generated_graphs]
    ...
    plt.legend(['Ground Truth', 'Baseline', 'Generated'])
    plt.title('max_degree')
    plt.xlabel('value')
    plt.savefig('max_degree')

    # mean centrality
    stats_test = [np.mean(list(nx.degree_centrality(G).values())) for G in test_graphs]
    stats_baseline = [np.mean(list(nx.degree_centrality(G).values())) for G in baseline_graphs]
    stats_generated = [np.mean(list(nx.degree_centrality(G).values())) for G in generated_graphs]
    ...
    plt.legend(['Ground Truth', 'Baseline', 'Generated'])
    plt.title('mean_centrality')
    plt.xlabel('value')
    plt.savefig('mean_centrality')

    # assortativity
    stats_test = [nx.degree_assortativity_coefficient(G) for G in test_graphs]
    stats_baseline = [nx.degree_assortativity_coefficient(G) for G in baseline_graphs]
    stats_generated = [nx.degree_assortativity_coefficient(G) for G in generated_graphs]
    ...
    stats_test = [np.mean(list(nx.average_degree_connectivity(G).values())) for G in test_graphs]
    stats_baseline = [np.mean(list(nx.average_degree_connectivity(G).values())) for G in baseline_graphs]
    stats_generated = [np.mean(list(nx.average_degree_connectivity(G).values())) for G in generated_graphs]
    ...
    plt.legend(['Ground Truth', 'Baseline', 'Generated'])
    plt.title('mean_degree_connectivity')
    plt.xlabel('value')
    plt.savefig('mean_degree_connectivity')
