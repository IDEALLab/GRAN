...
__all__ = ['save_graph_list', 'load_graph_list', 'graph_load_batch', 'preprocess_graph_list', 'create_graphs']
...
def graph_load_batch(data_dir,
    ...
    print('data_adj:', data_adj.shape, data_adj[:5])
    if node_attributes:
        data_node_att = np.loadtxt(os.path.join(path, '{}_node_attributes.txt'.format(name)), delimiter=',')
    data_node_label = np.loadtxt(os.path.join(path, '{}_node_labels.txt'.format(name)), delimiter=',').astype(int)
    print('data_node_label:', data_node_label.shape, data_node_label[:5])

    data_graph_indicator = np.loadtxt(
        os.path.join(path, '{}_graph_indicator.txt'.format(name)), delimiter=',').astype(int)
    print('data_graph_indicator:', data_graph_indicator.shape, data_graph_indicator[:5])

    if graph_labels:
        data_graph_labels = np.loadtxt(
            os.path.join(path, '{}_graph_labels.txt'.format(name)), delimiter=',').astype(int)

    data_tuple = list(map(tuple, data_adj))
    print('created data_tuple with length:', len(data_tuple), data_tuple[:5])

    # add edges
    G.add_edges_from(data_tuple)
    print('created G -> # nodes = {}, # edges = {}'.format(G.number_of_nodes(), G.number_of_edges()))
    ...
    print('removed isolates -> # nodes = {}, # edges = {}'.format(G.number_of_nodes(), G.number_of_edges()))

    # remove self-loop
    G.remove_edges_from(nx.selfloop_edges(G))
    print('removed self-loops -> # nodes = {}, # edges = {}'.format(G.number_of_nodes(), G.number_of_edges()))
...
def create_graphs(graph_type, data_dir='data', noise=10.0, seed=1234):
    ...
    if graph_type == 'grid':
        ...
        graphs = graph_load_batch(
            data_dir, min_num_nodes=100, max_num_nodes=500, name='DD', node_attributes=False, graph_labels=True)
        ...
        graphs = graph_load_batch(
            ...
            graph_labels=True)
    elif graph_type == 'LENS':
        graphs = graph_load_batch(
            data_dir, min_num_nodes=0, max_num_nodes=10000, name='LENS', node_attributes=False, graph_labels=False)
