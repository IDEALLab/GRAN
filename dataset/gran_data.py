...
class GRANData(object):
    def __init__(self, config, graphs, tag='train'):
        ...
        self.max_node_label = config.dataset.max_node_label
        # print('GRANData.__init__: self.max_node_label = {}'.format(self.max_node_label))  # 8
        ...
        self.save_path = os.path.join(self.data_path, '{}_{}_{}_{}_{}_{}_{}_precompute'.format(
            config.model.name, config.dataset.name, tag, self.block_size, self.stride, self.num_canonical_order,
            self.node_order))
        ...
        if not os.path.isdir(self.save_path) or self.is_overwrite_precompute:
            ...
            for index in tqdm(range(self.num_graphs)):
                G = self.graphs[index]
                data = self._get_graph_data(G)  # A list of adjacency matrices. Related with canonical ordering.
    ...
    def _get_graph_data(self, G):
        ...
        adj_1 = np.array(nx.to_numpy_matrix(G, nodelist=[dd[0] for dd in degree_sequence]))

        '========XD========'
        node_label_mat = np.zeros(shape=(G.number_of_nodes(),
                                         self.max_node_label + 1))  # each row is the one-hot form of node label
        for i in range(node_label_mat.shape[0]):
            nn = degree_sequence[i][0]
            node_label_mat[i, int(G.nodes[nn]['label'])] = 1
        adj_1 = np.concatenate((adj_1, node_label_mat), axis=1)
        '========XD========'

        ### Degree ascent ranking
        degree_sequence = sorted(node_degree_list, key=lambda tt: tt[1])
        adj_2 = np.array(nx.to_numpy_matrix(G, nodelist=[dd[0] for dd in degree_sequence]))
        ...
        for i in range(node_label_mat.shape[0]):
            nn = degree_sequence[i][0]
            node_label_mat[i, int(G.nodes[nn]['label'])] = 1
        adj_2 = np.concatenate((adj_2, node_label_mat), axis=1)
        '========XD========'
        ...
        for i in range(node_label_mat.shape[0]):
            nn = node_list_bfs[i]
            node_label_mat[i, int(G.nodes[nn]['label'])] = 1
        adj_3 = np.concatenate((adj_3, node_label_mat), axis=1)
        ...
        for i in range(node_label_mat.shape[0]):
            nn = node_list_dfs[i]
            node_label_mat[i, int(G.nodes[nn]['label'])] = 1
        # print('adj_4.shape = {}, node_label_mat.shape = {}'.format(adj_4.shape, node_label_mat.shape))
        adj_4 = np.concatenate((adj_4, node_label_mat), axis=1)
        # print('adj_4.shape = {}'.format(adj_4.shape))
        # return
        '========XD========'
        ...
        for kk in core_order_list:
            sort_node_tuple = sorted(
                [(nn, degree_dict[nn]) for nn in core_to_node[kk]], key=lambda tt: tt[1], reverse=True)
        ...
        for i in range(node_label_mat.shape[0]):
            nn = node_list[i]
            node_label_mat[i, int(G.nodes[nn]['label'])] = 1
        adj_5 = np.concatenate((adj_5, node_label_mat), axis=1)
        '========XD========'

        '========XD========'
        'nx.to_numpy_matrix() discards the node labels as well as node attributes/features.'
        '========XD========'
    ...
    def __getitem__(self, index):
        ...
        # load graph
        # print(index, self.file_names[index])
        adj_list = pickle.load(open(self.file_names[index], 'rb'))
        '========XD========'
        # print(len(adj_list), adj_list)  # a list which contains ONE 2d ndarray in the current setting: DFS node_order
        n = adj_list[0].shape[0]
        temp = [adj[:, :n] for adj in adj_list]
        node_label_list = [adj[:, n:] for adj in adj_list]
        adj_list = temp
        '========XD========'

        num_nodes = adj_list[0].shape[0]
        num_subgraphs = int(np.floor((num_nodes - K) / S) + 1)  #XD: 'num_subgraphs' means #blocks for this graph?
        ...
        for ff in range(self.num_fwd_pass):
            ...
            for ii in range(len(adj_list)):
                ...
                # print('size of adj_full:', adj_full.shape)
                # adj_tril = np.tril(adj_full, k=-1)

                node_label_full = node_label_list[ii]
                ...
                for jj in range(0, num_nodes, S):
                    ...
                    adj_block = np.pad(
                        adj_full[:jj, :jj], ((0, K), (0, K)), 'constant',
                    ...
                    if jj == 0:
                        ...
                        att_idx += [
                            np.concatenate([np.zeros(jj).astype(np.uint8),
                    ...
                    if jj == 0:
                        ...
                        node_idx_feat += [np.concatenate([np.arange(jj) + ii * N, np.ones(K) * np.inf])]

                    ### get node index for GNN output
                    idx_row_gnn, idx_col_gnn = np.meshgrid(np.arange(jj, jj + K), np.arange(jj + K))
                    idx_row_gnn = idx_row_gnn.reshape(-1, 1)
                    idx_col_gnn = idx_col_gnn.reshape(-1, 1)
                    node_idx_gnn += [np.concatenate([idx_row_gnn, idx_col_gnn], axis=1).astype(np.int64)]

                    ### get predict label
                    label += [adj_full[idx_row_gnn, idx_col_gnn].flatten().astype(np.uint8)]

                    ### get ground truth node label matrix
                    node_label_gt += [node_label_full[:jj + K, :]]
            ...
            ### pack tensors
            data = {}
            data['adj'] = np.tril(np.stack(adj_list, axis=0), k=-1)  #XD: (C, n, n)

            data['node_label'] = np.stack(node_label_list, axis=0)  #XD: (C, n, L)
            # print('__getitem__: node_label.shape = {}'.format(data['node_label'].shape))
            # return
            data['node_label_gt'] = np.concatenate(node_label_gt)
    ...
    def collate_fn(self, batch):
        # XD: this one is used for dataloader, not dataset.
        ...
        for ff in range(self.num_fwd_pass):
            ...
            subgraph_idx_base = np.array([0] + [bb['subgraph_count'] for bb in batch_pass])
            subgraph_idx_base = np.cumsum(subgraph_idx_base)

            data['subgraph_idx_base'] = torch.from_numpy(subgraph_idx_base)

            data['num_nodes_gt'] = torch.from_numpy(np.array([bb['num_nodes'] for bb in batch_pass])).long().view(-1)

            data['adj'] = torch.from_numpy(
                np.stack(
                    [
                        np.pad(
                            bb['adj'], ((0, 0), (0, pad_size[ii]), (0, pad_size[ii])), 'constant', constant_values=0.0)
            ...
            data['node_label'] = torch.from_numpy(
                np.stack(
                    [
                        np.pad(bb['node_label'], ((0, 0), (0, pad_size[ii]), (0, 0)))
                        for ii, bb in enumerate(batch_pass)
                    ],
                    axis=0)).float()  #XD: (B, C, N, L)

            idx_base = np.array([0] + [bb['num_count'] for bb in batch_pass])
            idx_base = np.cumsum(idx_base)

            data['edges'] = torch.cat([bb['edges'] + idx_base[ii] for ii, bb in enumerate(batch_pass)], dim=0).long()

            data['node_idx_gnn'] = torch.from_numpy(
                np.concatenate([bb['node_idx_gnn'] + idx_base[ii] for ii, bb in enumerate(batch_pass)],
                               axis=0)).long()

            data['att_idx'] = torch.from_numpy(np.concatenate([bb['att_idx'] for bb in batch_pass], axis=0)).long()

            # shift one position for padding 0-th row feature in the model
            node_idx_feat = np.concatenate(
                [bb['node_idx_feat'] + ii * C * N for ii, bb in enumerate(batch_pass)], axis=0) + 1
            ...
            data['label'] = torch.from_numpy(np.concatenate([bb['label'] for bb in batch_pass])).float()

            data['node_label_gt'] = torch.from_numpy(np.concatenate(
                [bb['node_label_gt'] for bb in batch_pass])).float()

            data['subgraph_idx'] = torch.from_numpy(
                np.concatenate(
                    [bb['subgraph_idx'] + subgraph_idx_base[ii] for ii, bb in enumerate(batch_pass)])).long()
